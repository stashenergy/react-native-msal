...
public class RNMSALModule extends ReactContextBaseJavaModule {
  ...
  public void createPublicClientApplication(ReadableMap params, Promise promise) {
    ...
    try {
      ...
      if(params.hasKey("androidMsal")) { // Get MSAL configs from runtime params
        JSONObject msalConfig = ReadableMapUtils.toJsonObject(params.getMap("androidMsal"));
        msalConfig.put("client_id", params.getMap("auth").getString("clientId"));

        // This was a must have param for this lib
        if(!msalConfig.has("account_mode")) {
          msalConfig.put("account_mode", "MULTIPLE");
        }

        InputStream inputStream = new ByteArrayInputStream(msalConfig.toString().getBytes("UTF-8"));
        FileUtils.copyInputStreamToFile(inputStream, file);
      } else { // Get MSAL configs from json
      ...
      promise.reject("Runtime MSAL config parse error; " + e);
    } catch (Exception e) {
      promise.reject(e);
    }
  }
